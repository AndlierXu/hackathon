...
std::string SSD::getResult(const std::vector<cv::Mat>& images, const caffe::Blob<Dtype>* detections,
                   const float threshold, const std::vector<cv::Scalar>& colors,
                   const std::map<int, std::string>& label_to_display_name)
{
  //std::cout<<detections->width()<<std::endl;
  //getchar();
 ...
 // float fps = num_img / (static_cast<double>(clock() - start_clock) /
  ...
  std::vector<caffe::LabelBBox> all_detections(num_img);
...
//int baseline = 0;
 // char buffer[50];
  ...
  for (int i = 0; i < num_img; ++i) {
    ...
    float mmaxscore=0.0;
     for (std::map<int, std::vector<caffe::NormalizedBBox> >::iterator it =
    ...
    //  CHECK_LT(label, colors.size());
     // const cv::Scalar& color = colors[label];
      const std::vector<caffe::NormalizedBBox>& bboxes = it->second;
      float maxscore=bboxes[0].score();
      for (int j = 0; j < bboxes.size(); ++j)
      {
        if(bboxes[j].score()>=maxscore)
        { 
           maxscore=bboxes[j].score();
           result=label_name;
        }
      }
      if(mmaxscore<maxscore)
      {
          mmaxscore=maxscore;
          result = label_name;
      }
...
std::string SSD::detect(const cv::Mat& img)
{
	//__TIC__();
	preprocess(img);
	std::string str;
	//printf("sg.xu: preprocess time: ");
	//__TOC__();
	// forward ....
	__TIC__();
	...
	if (config_.visualize)
	{
		...
		std::vector<cv::Scalar> colors = caffe::GetColors(label_to_display_name_.size());
                //std::cout<<label_to_display_name_.size()<<std::endl;
		caffe::VisualizeBBox(cv_imgs, output[0],config_. visualize_threshold, colors, label_to_display_name_);
		//printf("sg.xu: VisualizeBBox time: ");
		//__TOC__();

		//__TIC__();
                str=getResult(cv_imgs, output[0],config_. visualize_threshold, colors, label_to_display_name_);
		//printf("sg.xu: getResult time: ");
		//__TOC__();

                //printf("############\n");
                //std::cout<<str<<std::endl;
             
                //std::cout<<&label_to_display_name_->second<<std::endl;
              //  for(it=label_to_display_name_.begin();it!=label_to_display_name_.end();it++)
                //{
